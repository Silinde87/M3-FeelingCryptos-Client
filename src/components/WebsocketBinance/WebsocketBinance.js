...
class WebsocketBinance extends Component {
	...
	componentDidMount() {
		...
		this.client.readyState
			? this.client.send(`${this.props.market}`)
			: (this.client.onopen = () => this.client.send(`${this.props.market}`));
	...
	render() {
		return (
			<div id="profile-chart" style={{ width: "100%", marginLeft: "0px", minHeight: 315 }}>
				{this.state.charts.length > 0 && (
					<Chart data={this.state.charts} market={this.state.market} />
				)}
			</div>
		);
	}
